...
def test_list_view(request, course_pk=None):
    ...
    if course_pk is not None:
        ...
        if request.user.is_teacher():
            ...
            tests = Test.objects.filter(course=course, is_active=True).order_by('-created_at')
    else:
        if request.user.is_teacher():
...
@login_required
def test_create_view(request, course_pk):
...
def test_edit_view(request, test_id):
    ...
    if request.method == 'POST':
        form = TestCreateForm(request.POST, request.FILES, instance=test)
        if form.is_valid():
            form.save()
